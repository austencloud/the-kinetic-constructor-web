...
export class ModernLauncherState {
	...
	async refreshVersions(): Promise<void> {
		try {
			...
			if (this.isConnected) {
				realTimeService.broadcast('versions-updated', {
	...
	async startVersion(versionId: string): Promise<void> {
		...
		try {
			...
			if (this.isConnected) {
				realTimeService.broadcast('version-started', {
					versionId,
					name: version.name,
	...
	async stopVersion(versionId: string): Promise<void> {
		...
		try {
			...
			// Broadcast real-time event
			if (this.isConnected) {
				realTimeService.broadcast('version-stopped', {
					versionId,
					name: version.name
				});
			}
		...
		const updatePromises = Array.from(this.runningServers.keys()).map(async (versionId) => {
			try {
				...
				if (metrics) {
					...
					if (this.isConnected) {
						realTimeService.broadcast('performance-update', { versionId, metrics });
					}
				}
			} catch (error) {
		...
		const collectLogs = async () => {
			try {
				...
				if (this.isConnected && logs.length > 0) {
					...
					realTimeService.broadcast('logs-update', { versionId, logs: latestLogs });
				}
			} catch (error) {
