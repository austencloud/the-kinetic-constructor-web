<script lang="ts">
	...
	async function handleStart() {
		...
		try {
			...
			announceAction(`Failed to start ${version.name}: ${error.message}`, 'assertive');
		} finally {
			isStarting = false;
		}
	}
	...
	async function handleStop() {
		...
		try {
			...
			announceAction(`Failed to stop ${version.name}: ${error.message}`, 'assertive');
		} finally {
			isStopping = false;
		}
	}
	...
	async function handleRestart() {
		...
		try {
			...
			announceAction(`Failed to restart ${version.name}: ${error.message}`, 'assertive');
		}
	}
...
>
	...
	{#if isFocused}
		<div class="skip-link">
			<button onclick={() => cardElement.querySelector('.action-section button')?.focus()}>
				Skip to actions
			</button>
		</div>
	{/if}
				...
				{#if metrics}
					<div class="metrics-panel glass-card">
						<div class="grid grid-cols-2 gap-1 text-xs">
							...
							{#if metrics.cpu}
								<div class="metric-item" title="CPU usage: {metrics.cpu}%">
									<span class="text-slate-400" aria-label="CPU">
										<Cpu size={10} class="inline" aria-hidden="true" />
										CPU
									</span>
									<span class="text-neon-orange font-mono">{metrics.cpu}%</span>
								</div>
							{/if}
						</div>
					</div>
