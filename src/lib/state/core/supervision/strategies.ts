...
export class RestartStrategy extends BaseStrategy {
  ...
  async handleError(supervisor: Supervisor, actor: SupervisedActor<any>, error: Error): Promise<void> {
    ...
    this.failures.set(actor.id, list.filter(t => t >= start));
  ...
  private calcBackoff(attempt: number): number {
    switch (this.opts.backoffType) {
      case BackoffType.NONE:        return 0;
      case BackoffType.LINEAR:      return Math.min(this.opts.initialDelay * attempt, this.opts.maxDelay);
      case BackoffType.EXPONENTIAL: return Math.min(
      ...
      default:                      return this.opts.initialDelay;
    }
  }
...
export class ResumeStrategy extends BaseStrategy {
  ...
  async handleError(supervisor: Supervisor, actor: SupervisedActor<any>, error: Error): Promise<void> {
    ...
    list.push(now);
    const start = now - this.opts.withinTimeWindow;
    this.errors.set(actor.id, list.filter(t => t >= start));
