<script lang="ts">
  ...
  const persistentCount = createPersistentState('example_count', 0);
  
  // Example 4: Persistent object state with selective persistence
  const settings = createPersistentObjectState('example_settings', {
    theme: 'light',
    fontSize: 16,
...
<div class="example-container">
  ...
  <section>
    ...
    <button on:click={incrementPersistentCount}>Increment Persistent</button>
  </section>
  ...
  <section>
    ...
    <button on:click={() => settings.fontSize += 2}>Increase Font Size</button>
    <button on:click={() => settings.showNotifications = !settings.showNotifications}>
  ...
  <section>
    <h3>Reset All</h3>
    <button on:click={resetAll}>Reset All Values</button>
  </section>
</div>
