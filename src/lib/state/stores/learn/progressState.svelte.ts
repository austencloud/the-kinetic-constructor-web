		...
		updateLessonProgress: (
			...
			const avgScore =
				lessonsWithScores.length > 0
					? Math.round(
							lessonsWithScores.reduce((sum, l) => sum + l.lastScore, 0) / lessonsWithScores.length
						)
					: 0;
...
// Create the state
export const progressState = createProgressState();

// Derived getter for badge status
export const badgeStatus = $derived(() => {
	const badges: Record<string, 'none' | 'bronze' | 'silver' | 'gold'> = {};

	Object.entries(progressState.state.lessonProgress).forEach(([lessonId, progress]) => {
