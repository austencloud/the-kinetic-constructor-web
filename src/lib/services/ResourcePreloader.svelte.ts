// src/lib/services/ResourcePreloader.svelte.ts
...
// Create reactive state for the loading status (internal)
let _resourceLoadingStatus = $state<ResourceLoadingStatus>({
	loaded: 0,
	total: 0,
	failed: 0,
	inProgress: false,
...
// Export getter function for the loading status
export function getResourceLoadingStatus(): ResourceLoadingStatus {
	return _resourceLoadingStatus;
}
...
export class ResourcePreloader {
	...
	private updateStatus(updates: Partial<ResourceLoadingStatus>) {
		const newStatus = { ..._resourceLoadingStatus, ...updates };
		...
		_resourceLoadingStatus = newStatus;
	}
		...
		if (!this.resourcesFailed.has(resourceKey)) {
			...
			if (!isOptionalResource || !is404Error) {
				this.updateStatus({
					...
					errors: [..._resourceLoadingStatus.errors, { resource: resourceKey, error: errorMessage }]
				});
			} else {
	...
	async preloadProps(): Promise<void> {
		...
		this.updateStatus({
			...
			total: _resourceLoadingStatus.total + totalProps,
			inProgress: true
		});
	...
	async preloadArrows(): Promise<void> {
		...
		this.updateStatus({
			...
			total: _resourceLoadingStatus.total + totalArrows,
			inProgress: true
		});
	...
	async preloadGrids(): Promise<void> {
		...
		this.updateStatus({
			...
			total: _resourceLoadingStatus.total + gridTypes.length,
			inProgress: true
		});
	...
	async preloadGlyphs(): Promise<void> {
		...
		this.updateStatus({
			...
			total: _resourceLoadingStatus.total + totalGlyphs,
			inProgress: true
		});

		// Import the getLetterPath function from glyphState
		const { getLetterPath } = await import('$lib/stores/glyphState.svelte');
