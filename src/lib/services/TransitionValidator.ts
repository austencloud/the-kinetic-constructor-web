...
import {
	getValidTransitionsFromPosition,
...
export class TransitionValidator {
	...
	public async getValidTransitions(
		...
		console.log('🔍 TransitionValidator: Processing start position:', {
			letter: startPosition.letter,
			startPos: startPosition.startPos,
		...
		try {
			...
			if (filteredCount > 50) {
				warnings.push(
					`Large number of transitions found (${filteredCount}). Consider adding more filters.`
				);
			}
			...
			errors.push(
				`Failed to load transitions: ${error instanceof Error ? error.message : String(error)}`
			);
			return {
				isValid: false,
				validTransitions: [],
	...
	public async isValidTransition(
		...
		return result.validTransitions.some(
			(transition) =>
