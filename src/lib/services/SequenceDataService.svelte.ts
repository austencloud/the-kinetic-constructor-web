...
class SequenceDataService {
	...
	// Reactive state for the current sequence using Svelte 5 runes
	private sequenceState = $state<(SequenceMetadata | SequenceBeat)[]>([this.defaultMetadata]);
	...
	async saveSequence(sequence?: (SequenceMetadata | SequenceBeat)[]) {
		...
		try {
			...
			// Update the state
			this.sequenceState = sequenceToSave;
		} catch (error) {
	...
	// Get reactive access to the sequence state
	getSequenceState() {
		return () => this.sequenceState;
	}
