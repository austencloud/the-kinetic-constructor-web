...
function generateOrientation(options: { propContinuity: 'continuous' | 'random' }) {
	const generateSingleOrientation = () => ['in', 'out', 'cw', 'ccw'][Math.floor(Math.random() * 4)];
...
function determinePosition(index: number): string {
	const positions = ['alpha1_alpha1', 'beta5_beta5', 'gamma11_gamma11'];
...
export function transformSequence(sequence: Beat[], transformationType: string): Beat[] {
