...
<script lang="ts">
  ...
  import { isWebShareSupported, isFileShareSupported } from '$lib/components/SequenceWorkbench/share/utils/shareUtils';
  ...
  async function shareSequence(): Promise<boolean> {
    ...
    if (timeSinceLastAttempt < MIN_SHARE_INTERVAL_MS) {
      console.log(`ShareNative: Share attempted too soon (${timeSinceLastAttempt}ms since last attempt)`);
    ...
    try {
      ...
      if (timeSinceLastApiCall < MIN_SHARE_API_INTERVAL_MS) {
        console.log(`ShareNative: Web Share API called too soon (${timeSinceLastApiCall}ms since last attempt)`);
        showError('Please wait a moment before sharing again');
        resetSharingState();
        return false;
      }
      ...
      if (fileShareSupported && result) {
        try {
          ...
          if (navigator.canShare && navigator.canShare(shareData)) {
            ...
            console.log('ShareNative: Device cannot share with files, falling back to text+URL only');
          ...
          console.warn('ShareNative: Error sharing with file, falling back to text+URL only:', fileError);
          // Fall through to text+URL sharing
        }
      }
