...
// Import LZString for decompression if available
 ...
 */
export function decodeSequenceCompact(encoded: string): BeatData[] {
    if (!encoded) return [];
 ...
 */
export function decodeMotion(encoded: string, color: 'blue' | 'red', defaultOri: string): MotionData | null {
 ...
 */
export function inferEndPosition(blueMotion: MotionData | null, redMotion: MotionData | null): string {
    if (!blueMotion || !redMotion) return 'alpha1';
