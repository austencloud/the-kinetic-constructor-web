...
import { renderSequenceToImage, type SequenceRenderResult } from './sequenceImageRenderer';
...
export async function testImageExport(): Promise<ImageExportTestResult> {
  ...
  try {
    ...
    testElement.innerHTML = '<p style="color: white; text-align: center; padding: 20px;">Test Image</p>';
...
export function dataURLtoBlob(dataUrl: string): Blob {
  try {
    ...
    try {
      ...
      throw new Error(`Failed to decode base64 data: ${decodeError instanceof Error ? decodeError.message : String(decodeError)}`);
    }
  } catch (error) {
