 ...
 */
export function findBeatFrameElement(beatFrameContext?: ElementContext | null): HTMLElement | null {
	...
	if (alternativeElement instanceof HTMLElement) {
		logger.debug('ElementFinder: Found alternative element');
 ...
 */
export function listenForBeatFrameElement(callback: (element: HTMLElement) => void): () => void {
	...
	document.addEventListener('beatframe-element-available', handleElementAvailable as EventListener);
