<!-- src/lib/components/SequenceWorkbench/RemoveStartPositionButton.svelte -->
<script lang="ts">
	...
	function handleClick() {
		// Provide error haptic feedback for deletion of start position
		if (browser) {
...
<button
	...
	onclick={handleClick}
	aria-label="Remove start position (clears entire sequence)"
...
>
	...
	<span class="button-text">Remove Start Position</span>
</button>
...
<style>
	.remove-beat-button {
		...
		--remove-beat-button-width: 220px; /* Export this for other components to use - wider for the longer text */
