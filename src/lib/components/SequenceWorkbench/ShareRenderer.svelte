...
<script lang="ts">
  ...
  import { renderSequence as renderSequenceUtil } from './utils/ShareSequenceRenderer';
  ...
  async function renderSequence(): Promise<SequenceRenderResult | null> {
    ...
    try {
      ...
      // Use the utility function to render the sequence
      const result = await renderSequenceUtil({
        beatFrameElement: props.beatFrameElement,
        contextElement: cachedBeatFrameElement,
        sequenceName,
        currentWord
      });
