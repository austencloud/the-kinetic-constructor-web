<script lang="ts">
	...
	// Setup options for fly transition
	function getItemTransition(index: number) {
		// Calculate row and column for diagonal wave effect
		const columns = layout.gridColumns.match(/repeat\((\d+)/)?.[1];
		const numCols = columns ? parseInt(columns) : 4;
		const row = Math.floor(index / numCols);
		const col = index % numCols;

		// Diagonal wave pattern - items enter from slightly off their final position
		return {
			duration: 300,
			delay: (row + col) * 30, // Diagonal wave effect
			easing: cubicOut,
			x: 0,
			y: 20 // Slight vertical shift looks smooth
		};
	}
</script>
	...
	>
		{#each options as option, i (`${option.letter}-${option.startPos}-${option.endPos}-${i}`)}
			<div
				...
				in:fly={getItemTransition(i)}
				out:fade={{ duration: 200, delay: i * 20 }}
			>
