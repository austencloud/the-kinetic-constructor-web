<script lang="ts">
  ...
  function createDummyStartPosition(): Beat {
    ...
    beat.pictographData.startPos = 'start';
    return beat;
  }
  ...
  async function testLayout(container: HTMLElement, beatCount: number, includeStart: boolean, title: string) {
    try {
      // Clear container
      container.innerHTML = '';
