...
export function parseTurnsTupleString(
	...
	return [parseDirection(dirRaw), parseTurnValue(topRaw) ?? 0, parseTurnValue(bottomRaw) ?? 0];
}
...
function parseDirection(item: string | null): DirRelation | PropRotDir | null {
	...
	return validDirections.includes(item as any) ? (item as DirRelation | PropRotDir) : null;
}
...
function parseTurnValue(item: string | null): TKATurns | null {
	...
	return !isNaN(num) && VALID_TURN_NUMS.includes(num as ValidTurnNum) ? (num as TKATurns) : null;
}
