...
<script lang="ts">
  ...
  <g class="turns-number-group">
    {#if topValue !== 0}
      {#if (topDetails = getNumberSVGDetails(topValue))}
        <g transform={`translate(${turnsPositions.top.x}, ${turnsPositions.top.y})`}>
          <image
            href={topDetails.path}
            width={topDetails.width}
            height={topDetails.height}
          />
        </g>
      {/if}
    {/if}
    ...
    {#if bottomValue !== 0}
      {#if (bottomDetails = getNumberSVGDetails(bottomValue))}
