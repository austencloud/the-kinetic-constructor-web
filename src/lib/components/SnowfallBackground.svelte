<script lang="ts">
	...
	const createSantaSystem = () => {
		...
		const update = (
			...
			if (santa.active) {
				...
				if ((santa.direction === 1 && newX > 1.2) || (santa.direction === -1 && newX < -0.2)) {
					return {
	...
	const handleResize = (): void => {
		...
		// Adjust snowflakes
		snowflakes = snowflakeSystem.adjustToResize(snowflakes, oldDimensions, newDimensions);
	};
	...
	onMount(() => {
		...
		// Dispatch ready event immediately after initialization
		dispatch('ready');
