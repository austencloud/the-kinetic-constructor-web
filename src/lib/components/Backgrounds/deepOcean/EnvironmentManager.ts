...
export class EnvironmentManager {
    ...
    private initializeCoral(dimensions: Dimensions, quality: QualityLevel): void {
        ...
        for (let i = 0; i < count; i++) {
            ...
            this.coral.push({
                x, y, width, height, branches,
                color: this.config.coral.colors[Math.floor(Math.random() * this.config.coral.colors.length)],
    ...
    public draw(ctx: CanvasRenderingContext2D, dimensions: Dimensions, accessibility: AccessibilitySettings, quality: QualityLevel): void {
        this.drawLightRays(ctx, accessibility, quality);
        this.drawCoral(ctx, accessibility);
        this.drawCaustics(ctx, dimensions, quality);
    }

    private drawLightRays(ctx: CanvasRenderingContext2D, accessibility: AccessibilitySettings, quality: QualityLevel): void {
        if (quality === 'low') return;
    ...
    private drawCaustics(ctx: CanvasRenderingContext2D, dimensions: Dimensions, quality: QualityLevel): void {
        if (quality === 'low') return;
        ...
        for (let x = 0; x < dimensions.width; x += causticSize) {
            for (let y = 0; y < dimensions.height; y += causticSize) {
                const intensity = Math.sin(x * 0.01 + this.causticOffset) * 
                                 Math.cos(y * 0.008 + this.causticOffset * 0.7);
