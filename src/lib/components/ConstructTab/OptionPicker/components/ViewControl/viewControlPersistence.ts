...
export function saveViewOption(option: ViewOption): void {
  ...
  console.log(`Saved view option to localStorage: ${option.label}`);
}
...
export function getInitialViewOption(
  ...
  if (viewMode === 'all') {
    ...
    if (allOption) {
      console.log('Restored "Show All" view option from localStorage');
      return allOption;
    }
    ...
    if (sortMethod) {
      ...
      if (methodOption) {
        console.log(`Restored "${methodOption.label}" view option from localStorage`);
        return methodOption;
      }
    }
  }
