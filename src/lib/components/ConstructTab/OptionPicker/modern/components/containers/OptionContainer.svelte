...
<script lang="ts">
	...
	import type {
		...
		FilterCriteria,
		SortCriteria
	} from '../../services/core/IOptionService';
	...
	import ModernOptionGrid from '../presentation/ModernOptionGrid.svelte';
	...
	$effect(() => {
		...
		const unsubscribeOptionsLoaded = optionService.on('options:loaded', (data) => {
		...
		const unsubscribeOptionsFiltered = optionService.on('options:filtered', (data) => {
		...
		const unsubscribeError = optionService.on('error', (data) => {
	...
	const selectedOptionIds = $derived(
		selectedOptions.map((option) => `${option.letter}-${option.startPos}`)
	);
