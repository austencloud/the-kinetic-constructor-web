...
<script lang="ts">
  ...
  function isPositionSelected(position: PictographData): boolean {
    ...
    return selectedPosition.letter === position.letter && 
           selectedPosition.endPos === position.endPos;
  }
  ...
  function handleKeyDown(event: KeyboardEvent) {
    ...
    const currentIndex = selectedPosition ? 
      startPositions.findIndex(pos => isPositionSelected(pos)) : -1;
...
>
  ...
  <div class="sr-only" aria-live="polite">
    {#if hasPositions}
      ...
      {#if selectedPosition}
        Currently selected: {selectedPosition.letter || 'unknown'} at position {selectedPosition.endPos || 'unknown'}.
      {:else}
