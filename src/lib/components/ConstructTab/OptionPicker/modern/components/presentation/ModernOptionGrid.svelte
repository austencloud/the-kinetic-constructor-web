...
<script lang="ts">
  ...
  import ModernOptionCard from './ModernOptionCard.svelte';
  ...
  function isOptionSelected(option: PictographData): boolean {
    const optionId = `${option.letter}-${option.startPos}`;
    ...
    >
      {#if virtualScrolling}
        ...
        <div 
          ...
          style:height="{Math.ceil(options.length / gridLayout.columns) * (gridLayout.cellSize + gridLayout.gap)}px"
        ></div>
      {/if}
      ...
      <div 
        ...
        style:transform={virtualScrolling ? `translateY(${Math.floor(scrollTop / (gridLayout.cellSize + gridLayout.gap)) * (gridLayout.cellSize + gridLayout.gap)}px)` : 'none'}
        role="grid"
        aria-rowcount={Math.ceil(options.length / gridLayout.columns)}
        aria-colcount={gridLayout.columns}
      >
        {#each visibleOptions as option, index (option.letter + option.startPos)}
          {@const globalIndex = virtualScrolling ? 
          ...
          >
            <ModernOptionCard
              ...
              size={gridLayout.cellSize <= 80 ? 'small' : gridLayout.cellSize >= 160 ? 'large' : 'medium'}
