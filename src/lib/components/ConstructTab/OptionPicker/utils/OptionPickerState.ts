...
export function createDerivedUIStores(
	...
	const groupedOptions = derived(
		groupedOptionsStore,
		($groupedOptionsStore) => $groupedOptionsStore
	);

	const filteredOptions = derived(
		filteredOptionsStore,
		($filteredOptionsStore) => $filteredOptionsStore
	);

	const actualCategoryKeys = derived(groupedOptionsStore, ($groupedOptionsStore) =>
		$groupedOptionsStore ? Object.keys($groupedOptionsStore) : []
	);

	// Clear the loading state when options are loaded
	derived([isLoading, filteredOptionsStore], ([$isLoading, $filteredOptions]) => {
	...
	const showTabs = derived(
		[selectedTab, groupedOptionsStore, uiState],
		([$selectedTab, $groupedOptions, $uiState]) => {
			// SIMPLIFIED LOGIC: Only check if selectedTab is 'all'
			// This is the most reliable way to determine if we're in "Show All" mode
			const isShowingAll = $selectedTab === 'all';

			// Check if we have categories to show
			const hasCategories = $groupedOptions && Object.keys($groupedOptions).length > 0;

			// Show tabs ONLY if:
			// 1. We're NOT in "Show All" mode (selectedTab !== 'all')
			// 2. We have categories available to show
			const shouldShowTabs = !isShowingAll && hasCategories;

			// Clear, concise logging
			if (shouldShowTabs) {
				console.log(
					`TABS VISIBLE: Using ${$uiState.sortMethod} filter with ${Object.keys($groupedOptions || {}).length} categories`
				);
			} else if (isShowingAll) {
				console.log('HELPER TEXT VISIBLE: "Show All" is selected');
			} else if (!hasCategories) {
				console.log('HELPER TEXT VISIBLE: No categories available to show');
			}
