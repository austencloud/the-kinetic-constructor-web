...
export function directionTransition(node: HTMLElement, params: DirectionTransitionParams = {}) {
	...
	const dx = direction === 'forward' ? distance : direction === 'backward' ? -distance : 0;
 ...
 */
export function staggeredItemTransition(
	node: HTMLElement,
	params: { index: number; total: number } & TransitionParams
) {
 ...
 */
export function optionGridTransition(
	node: HTMLElement,
	params: TransitionParams & { isEntering?: boolean }
) {
 ...
 */
export function swipeFeedbackTransition(
	node: HTMLElement,
	params: { percent: number; direction: 'left' | 'right' }
) {
