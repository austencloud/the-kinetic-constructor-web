...
export function setupEventListeners(
	...
	if (optionPickerElement) {
		...
		// Listen for the option-picker-update event
		document.addEventListener('option-picker-update', (event) => {
			if (event instanceof CustomEvent) {
				// This event is used to force a UI update after changing the sort method
				// We'll handle it by re-triggering the view change handler
				handleViewChange(event as CustomEvent<any>);
			}
		});
	}
...
export function cleanupEventListeners(
	...
	// Remove the document-level event listener for option-picker-update
	document.removeEventListener('option-picker-update', (event) => {
		if (event instanceof CustomEvent) {
			handleViewChange(event as CustomEvent<any>);
		}
	});
 ...
 */
export function loadOptionsFromSequence(sequenceDataService: any, actions: any) {
