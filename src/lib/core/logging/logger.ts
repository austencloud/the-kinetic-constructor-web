...
import { DEFAULT_LOGGER_CONFIG, CORRELATION_ID_PREFIX, SESSION_ID } from './constants';
...
export class LoggerImpl implements Logger {
	...
	createChildLogger(source: string, context?: Partial<Omit<LoggerContext, 'source'>>): Logger {
		return new LoggerImpl(source, this.config, {
			...
			...(this.context.correlationId ? { parentCorrelationId: this.context.correlationId } : {}),
			...context
		});
	}
	...
	private createLogEntry(
		...
		const correlationId =
			params?.correlationId || this.context.correlationId || this.generateCorrelationId();
